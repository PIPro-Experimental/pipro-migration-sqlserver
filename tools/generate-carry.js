...
function importSql(t, schemaVar) {
  const isPayroll = t.schemaType === 'PAYROLL';
  const hasAuto = t.columns.some(c => c.type === 'AUTO_KEY');
  const hasPayrollCol = t.columns.some(c => c.name === 'payroll');
...
// ---------------------------------------------------------------- hop-2 import
function emitImport(tables, fileTitle, schemaVar, note) {
  const parts = [`-- ===========================================================================
-- ${fileTitle}
...
-- Runner variables: :${schemaVar} :tenant_schema :payroll_number
--
...
`];
  for (const t of tables) {
    parts.push(`-- ---- ${t.name} ----`);
    parts.push(importSql(t, schemaVar));
    parts.push('');
  }
  parts.push('COMMIT;');
  parts.push(`\\echo 'Done (${fileTitle.split(' ')[0]}):' :tenant_schema '<-' :${schemaVar}`);
  return parts.join('\n');
}
...
fs.writeFileSync(path.join(OUT, '50_legacy_carry_company.sql'),
  emitImport(companyTables, '50_legacy_carry_company.sql — interim COMPANY-schema tables', 'legacy_company_schema'));
fs.writeFileSync(path.join(OUT, '55_legacy_carry_payroll.sql'),
  emitImport(payrollTables, '55_legacy_carry_payroll.sql — interim PAYROLL-schema tables', 'legacy_payroll_schema',
